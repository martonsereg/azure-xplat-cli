...
exports.init = function (cli) {
  ...
  cloudService.command('deploy')
    .description($('Deploy a cloud service package'))
    .option('-c, --cloudService <cloudService>', $('the name of the cloud service'))
    .option('-d, --deploymentSlot <deploymentSlot>', $('staging or production'))
    .option('-p, --packageUrl <packageUrl>', $('the blob storage url of the package'))
    .option('-f, --configFile <configFile>', $('the local path of a .cscfg configuration file'))
    .option('-l, --deploymentLabel <deploymentLabel>', $('the deployment label'))
    .option('-s, --subscription <id>', $('the subscription id'))
    .execute(function (options, _) {
      var specifiedFileName = options['configFile']
      ...
      var content=fs.readFileSync(specifiedFileName, "utf8");
      
      var service = utils.createComputeClient(profile.current.getSubscription(options.subscription));
      var parameters = {
          name: options['cloudService'],
          label: options['deploymentLabel'],
          configuration: content,
          startDeployment: "true",
          packageUri: options['packageUrl']
      }

      var progress = cli.interaction.progress($('Deploying cloud service'));
      try {
        service.deployments.create(options['cloudService'], options['deploymentSlot'], parameters, _)
      } finally {
        progress.end();
      }
        
      log.info("Cloud service package successfully deployed.")
    });  
